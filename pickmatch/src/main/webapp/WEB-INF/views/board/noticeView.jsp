...
<div class="freeboard-container">
	<div class="freeboard-wrapper">
		<div class="freeboard-title">
			<div class="freeboard-maintitle">${notice.noticeTitle }</div>
			<div class="freeboard-subtitle">
				<div>
					<div class="freeboard-subtitle-name freeboard-subtitle-member">작성자<span>${notice.memberId }</span></div>
				</div>
				<div>
					<div class="freeboard-subtitle-name">작성일<span><fmt:formatDate value="${notice.noticeDate }" pattern="yyyy.MM.dd"/></span></div>
				</div>
			</div>
		</div>		
		...
		<div class="freeboard-attach-img">
			<c:if test="${attachmentList !=null }">
  			...
  			|| fn:substringAfter(attach.renamedFileName,'.')== 'png' || fn:substringAfter(attach.renamedFileName,'.')== 'PNG'}">
						<div>
							<img src="${path }/resources/upload/notice/${attach.renamedFileName}" title="${attach.originalFileName }" width="70px">
						</div>
					</c:if>
				</c:forEach>
			</c:if>
	...
	<c:if test="${loggedMember.memberId==notice.memberId  || loggedMember.memberId == 'admin'}">
...
<script>
	...
	{
		location.href='${path}/board/noticeUpdate?noticeNo=${notice.noticeNo}';
	}
